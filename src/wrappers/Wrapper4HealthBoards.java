...
public class Wrapper4HealthBoards extends WrapperBase {
	...
	private void registerAuthor(Post p, String name, String ID) {
		p.setAuthor(name);
		p.setAuthorID(ID);
	...
	private Post extractPost(Element elm) {
		...
		if (tmpElmB!=null) {
			registerAuthor(p, tmpElmB.text(), tmpElmB.attr("href"));	
		} else {
			tmpElmB = tmpElmA.getElementsByTag("div").first();
			registerAuthor(p, tmpElmB.text(), tmpElmB.text());//using author's name as its ID
		}
...
//		wrapper.save2Json("./data/json/healthboards/Arthritis/sample.json");
		...
		wrapper.parseHTML("./data/HTML/healthboards/Drug-interactions-side-effects/error.htm");
		wrapper.save2Json("./data/json/healthboards/Drug-interactions-side-effects/error.json");
	}
}
