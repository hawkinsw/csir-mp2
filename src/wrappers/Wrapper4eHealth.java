...
public class Wrapper4eHealth extends WrapperBase {
	...
	private Post extractPost(Element elm) {
		//look for the element indicating if this element containing the right discussion posts
		...
		if (tmpElmA == null)
			return null;//No post id available, problematic post 
	...
	protected boolean parseHTML(Document doc) {
		...
		if (p!=null)
			...
			return false;//if the first post is missing, we will ignore the whole thread directly
	...
	public static void main(String[] args) {
		...
		wrapper.parseHTML("./data/HTML/eHealth/removed-discussion.htm");
		wrapper.save2Json("./data/json/eHealth/removed-discussion.json");
	}
